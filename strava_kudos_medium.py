...
def plot(data):
    ...
    for rect in rects:
        ...
        ax.text(rect.get_x() + rect.get_width() / 2, height + 1, int(height), ha='center', va='bottom', size=8)  
    ...
    ax.text(x=0.12, y=.91, s="Strava Kudoers", transform=fig.transFigure, ha='left', fontsize=13, weight='bold', alpha=.8)
    ax.text(x=0.12, y=.87, s="Number of kudos given from 01/01/2022 to 31/12/2022", transform=fig.transFigure, ha='left', fontsize=11, alpha=.8)
    
    # Adjust the spaces and margins of the plots
    plt.subplots_adjust(left=None, bottom=0.2, right=None, top=0.85, wspace=None, hspace=None)
    
    # Export plot as high resolution PNG
    plt.savefig('strava_kudos.png', dpi = 300, bbox_inches="tight", facecolor='white', pad_inches=0.5)
